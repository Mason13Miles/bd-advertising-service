...
/**
 * Adds a new targeting group to an existing piece of advertising content based on the contentId specified.
 * If a list of targeting predicates is provided the initial targeting group will be created with those rules.
 * Otherwise, the targeting group will be created without any predicates, meaning it is viewable by any customer.
 * Targeting groups are given a click through rate of 1 to start, so that they are guaranteed some initial
 * impressions and a true clickThroughRate can be learned.
 */
public class AddTargetingGroupActivity {
    ...
    public AddTargetingGroupResponse addTargetingGroup(AddTargetingGroupRequest request) {
        ...
        LOG.info(String.format("Adding targeting predicates [%s] to content with id: %s.",
                requestedTargetingPredicates, contentId));

        // Convert Coral TargetingPredicates to internal TargetingPredicates with a stream
        List<TargetingPredicate> targetingPredicates = Optional.ofNullable(requestedTargetingPredicates)
                .orElse(Collections.emptyList()) // handle null list
                .stream()
                .map(TargetingPredicateTranslator::fromCoral)
                .collect(Collectors.toList());

        // Create the new TargetingGroup
